...
class Visualizer():
    ...
    def create_landmarks_plot(self,
        ...
        # Create a color map with enough colors for all landmarks
        num_landmarks = len(self.landmarks.keys())
        colors = px.colors.qualitative.Alphabet * (num_landmarks // len(px.colors.qualitative.Alphabet) + 1)
        landmark_colors = dict(zip(self.landmarks.keys(), colors[:num_landmarks]))

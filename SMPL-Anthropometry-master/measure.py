...
def create_model(model_type, model_root, gender, num_betas=10, num_thetas=24):
    ...
    :param model_root: str of location where there are smpl/smplx/etc. folders with .pkl or .npz models
    ...
    # 构建完整的模型文件路径
    gender = gender.upper()
    # 确保model_root指向具体的模型目录
    model_dir = os.path.join(model_root, model_type)  # 拼接model_root和model_type
    npz_model_path = os.path.join(model_dir, f'{model_type.upper()}_{gender}.npz')
    pkl_model_path = os.path.join(model_dir, f'{model_type.upper()}_{gender}.pkl')
    
    print(f"[create_model] 模型类型: {model_type}")
    print(f"[create_model] 模型根目录: {model_root}")
    print(f"[create_model] 性别: {gender}")
    print(f"[create_model] 模型目录: {model_dir}")
    print(f"[create_model] 尝试加载.npz模型文件: {npz_model_path}")
    
    # 检查路径是否存在
    if not os.path.exists(model_dir):
        print(f"[create_model] 模型目录不存在: {model_dir}")
    
    # 优先尝试加载.npz格式的模型文件
    if os.path.exists(npz_model_path):
        print(f"[create_model] 成功找到.npz模型文件: {npz_model_path}")
        try:
            # 直接使用模型文件路径，而不是目录
            model = smplx.create(model_path=npz_model_path, 
                                model_type=model_type, 
                                gender=gender, 
                                use_face_contour=False, 
                                num_betas=num_betas)
            print(f"[create_model] 成功创建.npz模型")
            return model
        except Exception as e:
            print(f"[create_model] 创建.npz模型时出错: {e}")
    elif os.path.exists(pkl_model_path):
        print(f"[create_model] 成功找到.pkl模型文件: {pkl_model_path}")
        try:
            # 直接使用模型文件路径，而不是目录
            model = smplx.create(model_path=pkl_model_path, 
                                model_type=model_type, 
                                gender=gender, 
                                use_face_contour=False, 
                                num_betas=num_betas)
            print(f"[create_model] 成功创建.pkl模型")
            return model
        except Exception as e:
            print(f"[create_model] 创建.pkl模型时出错: {e}")
    else:
        error_msg = f"[create_model] 既找不到.npz模型文件: {npz_model_path}\n也找不到.pkl模型文件: {pkl_model_path}"
        print(error_msg)
...
class MeasureSMPL(Measurer):
    ...
    def __init__(self):
        ...
        self.body_model_root = r"c:\Users\lihui\Desktop\AiFashion\code\body-model-visualizer\data"
        self.body_model_path = os.path.join(self.body_model_root, 
                                            "body_models")

        # 使用create_model函数加载模型以支持.npz格式
        model = create_model(model_type=self.model_type, 
                             model_root=self.body_model_path, 
                             gender="NEUTRAL",
                             num_betas=10, 
                             num_thetas=self.num_joints)
        self.faces = model.faces
        face_segmentation_path = os.path.join(self.body_model_path, self.model_type,
...
class MeasureSMPLX(Measurer):
    ...
    def __init__(self):
        ...
        self.body_model_root = r"c:\Users\lihui\Desktop\AiFashion\code\body-model-visualizer\data\body_models"
